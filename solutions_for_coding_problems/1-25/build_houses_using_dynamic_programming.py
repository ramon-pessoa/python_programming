'''
Problem #19 [Medium]: A builder is looking to build a row of N houses that can be of K different colors. 
...
We can maintain a matrix cache where every entry [i][j] represents the minimum cost of painting house 
i the color j, as well as painting every house < i. 

We can calculate this by looking at the minimum cost of painting each house < i - 1, and painting house 
i - 1 any color except j, since that would break our constraint. 

We'll initialize the first row with zeroes to start. 

Then, we just have to look at the smallest value in the last row of our cache, since that represents the 
minimum cost of painting every house.

>> build_houses1(): This solution runs in O(N * K^2) time and O(N * K) space.



First off, notice that we're only ever looking at the last row when computing the next row's cost. 
That suggests that we only need to keep track of one array of size K instead of a whole matrix of size N * K.

>> build_houses2(): This solution is only using O(K) space!



When we're looking at the previous row's total cost, it looks like we're almost computing the same thing each time: 
...
>> build_houses3(): Now the runtime is only O(N * K) and the space complexity is O(1) - constant, since we keep track of only three variables!
'''

#########################################
def build_houses1(matrix):
	n = len(matrix)
	k = len(matrix[0])
	solution_matrix = [[0] * k]
...
#########################################
from math import inf # python >= 3.5
