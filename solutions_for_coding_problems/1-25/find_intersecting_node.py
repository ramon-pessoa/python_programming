'''
Problem #20 [Easy]: Given two linked lists that intersect at some point, find the intersection node. The lists are non-cyclical.
...
Naively, we could iterate through one of the lists and add each node to a set or dictionary, then we could iterate over the other list and check each node we're looking at to see if it's in the set. 

Then we'd return the first node that is present in the set. 
