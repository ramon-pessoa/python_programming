'''
Problem #58 [Medium]: An sorted array of integers was rotated an unknown number of times. 
...
However, we need to do it faster than linear time. 
A big clue should be that the array of integers was previously sorted, and then rotated. 
If it was just sorted, we could do a binary search. 
However, this array was also rotated, so we can't do a regular binary search. 
We can modify it slightly to get to where we want it, however.

Solution: we first find the rotation point using binary search. We do this by:
...
def shifted_array_search(lst, num):
	...
	# Now that we have the bottom, we can do binary search as usual, wrapping around the rotation.
	low = i
	high = i - 1
	dist = len(lst) // 2
	while True:
...
print("Note: Algorithm to find an element in an array faster than linear time")
arr = [13, 18, 25, 2, 8, 10]
num = 8

print("Given the array {0}, the index of the element {1} is {2}.".format(arr, num, shifted_array_search(arr, num)))
