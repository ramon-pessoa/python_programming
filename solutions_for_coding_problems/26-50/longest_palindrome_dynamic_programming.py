...
'''
The solution in longest_palindrome_brute_force.py run in O(N^3) time. 

We can improve the running time of this algorithm by using dynamic programming to store any repeated computations. 
Let's keep an N by N table A, where N is the length of the input string.
Then, at each index A[i][j] we'll keep whether or not the substring made from s[i:j] is a palindrome.
We'll make use of the following relationships:
...
We can see from this table that the longest palindromic substring here is "anana", since A[1:5] is the longest substring that's true in the table.
