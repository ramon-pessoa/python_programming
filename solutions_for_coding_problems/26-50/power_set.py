'''
Problem #37 [Easy]: The power set of a set is the set of all its subsets. Write a function that, given a set, generates its power set.
...
What's the pattern?
Notice that going from the empty set to {a}, that we still keep the empty set in our result and have another set with a in it. 
Similarly, when going from one element to two, we keep the same result set with one element ({}, {a}), but we also have a duplicate set with the b in it ({b}, {a ,b}).
