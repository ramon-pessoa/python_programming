'''
Problem #41 [Medium]: Given an unordered list of flights taken by someone, each represented as (origin, destination) pairs, and a starting airport, compute the person's itinerary. If no such itinerary exists, return null. If there are multiple possible itineraries, return the lexicographically smallest one. All flights must be used in the itinerary.
...
'''
This problem is similar to the N queens problem: we have a desired final state (all the flights are used up), we can construct partial itineraries and reject them, and at each step we have potentially multiple avenues to explore. 
...
To speed this up, we'll store all the flights into a dictionary with the origin as a key and a list of flight destinations from that origin as the value. 
Then we can look up our options in O(1) time instead of O(N) time.
'''
...
print("Person's itinerary for the list of flights [('SFO', 'HKO'), ('YYZ', 'SFO'), ('YUL', 'YYZ'), ('HKO','ORD')] and starting airport 'YUL' : %s" % get_itinerary([('SFO', 'HKO'), ('YYZ', 'SFO'), ('YUL', 'YYZ'), ('HKO', 'ORD')], 'YUL'))

print("Person's itinerary for the list of flights [('SFO', 'COM'), ('COM', 'YYZ')] and starting airport 'COM' : %s" % get_itinerary([('SFO', 'COM'), ('COM', 'YYZ')], 'COM'))

print("Person's itinerary for the list of flights [('A', 'B'), ('A', 'C'), ('B', 'C'), ('C', 'A')] and starting airport 'A' : %s" % get_itinerary([('A', 'B'), ('A', 'C'), ('B', 'C'), ('C', 'A')], 'A'))